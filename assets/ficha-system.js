...
import {get,ref,push,set,remove,update,onValue} from 'https://www.gstatic.com/firebasejs/12.18.0/firebase-database.js';
...
function progenitorOptions(){return Object.entries(affiliations).filter(([,a])=>a?.studio===data.affiliationStudio&&a?.story===data.affiliationStory).sort((a,b)=>String(a[1]?.name||'').localeCompare(String(b[1]?.name||''),'pt-BR')).map(([key,a])=>{const max=Number(a.maxChildren??a.maxFilhos??3)||3;const rawCount=Number(slotCounts[key]??a.currentChildren??0)||0;const isCurrent=editingExisting&&data.affiliationKey===key;const count=Math.max(0,rawCount-(isCurrent?1:0));const manual=a.manualUnavailable===true;const full=count>=max;const unavailable=manual||full;const reason=manual?'Bloqueio manual':full?'Lotada':'';const label=`${String(count).padStart(2,'0')}/${String(max).padStart(2,'0')} — ${unavailable?'Indisponível':'Disponível'}${reason?' · '+reason:''}`;return `<option value="${esc(a.name)}" data-key="${esc(key)}" ${unavailable?'disabled':''} ${data.progenitors===a.name&&!unavailable?'selected':''}>${esc(a.name)} · ${label}</option>`}).join('')}
...
async function load(){
  try{
    ...
    affiliations=a?.val()||{};
    // O Firebase permanece como fonte oficial enquanto a ficha estiver aberta.
    // Alterações administrativas de disponibilidade chegam em tempo real.
    onValue(ref(db,'site/filiacoes'), snap => {
      affiliations=snap.val()||{};
      if($('formWrap') && !$('formWrap').hidden && step===1) render();
    }, err => console.warn('[ficha] Falha ao sincronizar filiações em tempo real:', err));
    activities=b?.val()||{};
